...
enum NUTRIENTS {
	Cal, Carb, Fat, Pro, Fib
};

/**
 * * This class creates the javaFx screen, creates the mealList, foodList,
 * query, addFood, and chooseMeal scenes entities are created and added to the
 * scenes, and the scenes are all linked. There are popups for errors. This
 * displays the screen and lets users add food items, remove food items, create
 * meals, analyze meals, and search for food by nutrients and by name.
 * 
 * @authors Thomas Antonacci, Sally Gerich, Kelsey Hickok, William Hofkamp,
 *          Apostolos Velliotis
 */
public class Main extends Application {
	...
	/**
	 * This method creates the stages and GUI layout for the Main It loads all
	 * layouts and creates the functionality for the buttons and lists
	 * 
	 * @param Stage
	 *            primaryStage
	 */
	public void start(Stage primaryStage) throws Exception {
		...
		mealLayout.getChildren().addAll(mealN, mealIn, mealCb, mealCa, mealPr, mealFa, mealFi, text);
		...
		// creates a tool tip to give instructions to the user when the mouse is over
		// the button
		...
		unfilter.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				// if clicked, will unfilter the button from what was filtered on the query
				// screen
				queryOn = false;
		...
		// creates a tool tip to inform the user what to do when the mouse is over the
		// button
		...
		fileButton.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(final ActionEvent e) {
				...
				// if the file Button is selected, load a file chooser and pass the selected
				// file to a variable
		...
		entrBtn.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent e) {
				...
				if (foodF.getText().isEmpty() || carbF.getText().isEmpty() || calF.getText().isEmpty()
						|| fatF.getText().isEmpty() || proF.getText().isEmpty() || fibF.getText().isEmpty()) {
		...
		chooseTable.setOnMouseClicked((new EventHandler<MouseEvent>() {
			@Override
			public void handle(MouseEvent event) {
				// if a meal is double clicked, keep the selected meal and load the food screen
				// to select food to add to that meal
		...
		continueButton.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				// if the button is clicked, keep the selected meal and load the food screen to
				// select food to add to that meal
				chosenMeal = (MealItem) chooseTable.getSelectionModel().getSelectedItem();
				if (chosenMeal != null) {
					currentMeal.setText(chosenMeal.getName());
					primaryStage.setScene(foodScreen);
		...
		addMealButton.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				// if the text is empty and the add Meal button is pressed, display a pop up
				// warning
		 ...
		 * homeScreen.getStylesheets().add(getClass().getResource("../styleFile.css").
		 * toExternalForm());
		 * foodScreen.getStylesheets().add(getClass().getResource("../styleFile.css").
		 * toExternalForm());
		 * queryScreen.getStylesheets().add(getClass().getResource("../styleFile.css").
		 * toExternalForm());
		 * addScreen.getStylesheets().add(getClass().getResource("../styleFile.css").
		 * toExternalForm());
		 * mealScreen.getStylesheets().add(getClass().getResource("../styleFile.css").
		 * toExternalForm());
		 * chooseScreen.getStylesheets().add(getClass().getResource("../styleFile.css").
		 * toExternalForm());
		 */
